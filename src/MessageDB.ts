...
export default class MessageDB {
  ...
  addNewMessage = (message: Message) => {
    ...
    this.client.set("message" + message.id.toString(), JSON.stringify(message));
  ...
  settleMessageWithInvoice = (
    invoice: string,
    fnMsgId: Function,
    fnSatCounter: Function
  ) => {
      ...
      if (err) {
        ...
        this.client.get("message" + id, (err, message) => {
          if (err) {
            ...
            if (!msgJson) {
              console.log("ERROR: msgJson null", id, message);
              return;
            }
